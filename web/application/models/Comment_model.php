...
class Comment_model extends Emerald_Model {
    ...
    public static function preparation_tree(array $collection)
    {
        $dataset = [];
        ...
        foreach ($dataset as $id => &$node) {
            if (isset($node['reply_id']) && $node['reply_id'] == $parentId) {
                $tree[] = &$node;
            } elseif (isset($node['reply_id'])) {
                $dataset[$node['reply_id']]['children'][] = &$node;
            }
        }
    ...
    public static function findCommentsByPostId($postId): ?array
    {
        return App::get_s()
                ->from(self::CLASS_TABLE)
                ->leftJoin(Post_comment_model::CLASS_TABLE, ['comment.id' => 'post_comment.comment_id'])
                ->leftJoin(Post_model::CLASS_TABLE, ['post.id' => 'post_comment.post_id'])
                ->where(['post_comment.post_id' => (int) $postId])
                ->select('comment.*')
                ->many();
    }
}
